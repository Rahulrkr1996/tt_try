...
/* ================================================================
   SECTION 03 — PARTNERS · "Powered By Those Who Believe"
   Holographic sponsorship wall assembling in real time: light
   streaks, particle motion, a stabilising geometric grid, and glass
   panels emerging from three depth layers. Each logo stages a
   blur→sharp reveal with a subtle activation pulse. A small helmet
   drifts in the background as the narrative anchor.
================================================================ */
const PARTNERS = [
  { name: "Assam Petroleum", tier: "Principal Partner", layer: 0 },
  { name: "Brahmaputra Airlines", tier: "Title Sponsor", layer: 0 },
  { name: "TeaVerse", tier: "Kit Partner", layer: 1 },
  { name: "Northeast Bank", tier: "Official Partner", layer: 1 },
  { name: "Kaziranga Resorts", tier: "Hospitality", layer: 2 },
  { name: "Rhino Energy", tier: "Beverage", layer: 2 },
];

function HoloGrid({ progress }: { progress: MotionValue<number> }) {
  const opacity = useTransform(progress, [0, 0.25, 0.9, 1], [0, 0.5, 0.35, 0.25]);
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          backgroundImage:
            "linear-gradient(oklch(0.72 0.19 55 / 0.18) 1px, transparent 1px), linear-gradient(90deg, oklch(0.72 0.19 55 / 0.18) 1px, transparent 1px)",
          backgroundSize: "72px 72px",
          maskImage:
            "radial-gradient(ellipse 65% 55% at 50% 55%, black 40%, transparent 85%)",
          WebkitMaskImage:
            "radial-gradient(ellipse 65% 55% at 50% 55%, black 40%, transparent 85%)",
          transform: "perspective(1200px) rotateX(58deg) translateY(12%)",
...
function LightStreaks({ progress }: { progress: MotionValue<number> }) {
  const opacity = useTransform(progress, [0, 0.2, 0.6, 1], [0, 1, 0.6, 0.3]);
  return (
    <motion.div aria-hidden style={{ opacity }} className="pointer-events-none absolute inset-0 overflow-hidden">
      {[18, 42, 68, 84].map((top, i) => (
        <motion.div
          key={i}
          className="absolute h-px w-[140%]"
          style={{
            top: `${top}%`,
            left: "-20%",
            background:
              "linear-gradient(90deg, transparent, oklch(0.9 0.05 70 / 0.6), oklch(0.72 0.19 55 / 0.9), oklch(0.9 0.05 70 / 0.6), transparent)",
            filter: "blur(1px)",
          }}
          animate={{ x: ["-10%", "10%", "-10%"] }}
...
const LAYER_STYLE: Record<number, { depth: number; blur: number; scale: number; opacity: number }> = {
  0: { depth: 0, blur: 0, scale: 1, opacity: 1 },       // foreground
  1: { depth: -140, blur: 0.6, scale: 0.94, opacity: 0.92 }, // mid
  2: { depth: -280, blur: 1.6, scale: 0.86, opacity: 0.78 }, // background
};
...
function SponsorPanel({
  ...
  const style = LAYER_STYLE[partner.layer];
  // Staggered emergence: each panel wakes up in its own window
  const start = 0.28 + index * 0.06;
  const end = start + 0.22;
  const revealed = useTransform(progress, [start, end], [0, 1]);
  const panelY = useTransform(revealed, [0, 1], [80, 0]);
  const panelOpacity = useTransform(revealed, [0, 1], [0, style.opacity]);
  const panelBlur = useTransform(revealed, [0, 1], [14 + style.blur * 4, style.blur]);
  const logoBlur = useTransform(revealed, [0, 1], [10, 0]);
  const logoOpacity = useTransform(revealed, [0, 0.6, 1], [0, 0.4, 1]);
  const logoSat = useTransform(revealed, [0, 1], [0.2, 1]);
  const pulse = useTransform(revealed, [0.85, 0.95, 1], [0, 0.55, 0]);
  const panelFilter = useMotionTemplate`blur(${panelBlur}px)`;
  const logoFilter = useMotionTemplate`blur(${logoBlur}px) saturate(${logoSat})`;

  // Gentle continuous drift (broadcast-grade minimal amplitude)
  return (
    <motion.div
      style={{
        ...
        y: panelY,
        filter: panelFilter,
        transform: `translateZ(${style.depth}px) scale(${style.scale})`,
    ...
    >
      <motion.div
        animate={{ y: [0, -6, 0, 4, 0] }}
        transition={{ duration: 10 + (index % 3) * 2, repeat: Infinity, ease: "easeInOut" }}
        className="relative overflow-hidden rounded-3xl border border-white/12 bg-white/[0.05] p-8 backdrop-blur-xl"
      >
        {/* Activation pulse */}
        <motion.div
          aria-hidden
        ...
        >
          <div
            ...
            style={{
              boxShadow:
                "inset 0 0 60px oklch(0.72 0.19 55 / 0.55), 0 0 80px oklch(0.72 0.19 55 / 0.35)",
            }}
          />
        </motion.div>
        <div className="pointer-events-none absolute -inset-px rounded-3xl bg-gradient-to-br from-titans/25 via-transparent to-transparent opacity-70" />
        <div className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-titans/70 to-transparent" />
        <div className="relative">
          <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-titans">
            {partner.tier}
          </div>
          <motion.div
            style={{ opacity: logoOpacity, filter: logoFilter }}
            className="mt-6 font-display text-3xl leading-tight text-fire"
          >
            {partner.name}
          </motion.div>
          <div className="mt-10 flex items-center justify-between border-t border-white/10 pt-4">
            <span className="font-mono text-[9px] uppercase tracking-widest text-foreground/50">
              CH · {String(index + 1).padStart(2, "0")} / {String(PARTNERS.length).padStart(2, "0")}
            </span>
            <Handshake className="h-4 w-4 text-titans" />
          </div>
        </div>
      </motion.div>
    </motion.div>
...
function PartnersSection() {
  ...
  const atmosphereOpacity = useTransform(scrollYProgress, [0, 0.15, 1], [0, 1, 1]);
  const helmetOpacity = useTransform(scrollYProgress, [0.05, 0.25, 0.9, 1], [0, 0.35, 0.28, 0.22]);
  const helmetScale = useTransform(scrollYProgress, [0, 1], [0.7, 0.85]);
  const helmetRot = useTransform(scrollYProgress, [0, 1], [0, lite ? 120 : 300]);
  const helmetTransform = useMotionTemplate`translate(-50%, -50%) scale(${helmetScale}) rotateY(${helmetRot}deg)`;

  // Very subtle parallax across the panel grid
  const gridParallax = useTransform(scrollYProgress, [0.3, 1], [40, -40]);
  const headerY = useTransform(scrollYProgress, [0, 0.3], [40, 0]);
  const headerOpacity = useTransform(scrollYProgress, [0, 0.25], [0, 1]);

  return (
    <section id="partners" ref={ref} className="relative h-[320vh] bg-jet">
      <div className="sticky top-0 h-screen overflow-hidden">
        ...
        <div className="absolute inset-0">
          ...
          <div
            ...
            style={{
              background:
                "radial-gradient(ellipse 70% 60% at 50% 55%, oklch(0.14 0.05 55 / 0.55), oklch(0.04 0 0) 78%)",
            }}
          />
        </div>

        {/* Continuity anchor: subtle rotating helmet deep in scene */}
        <motion.div
          aria-hidden
          style={{ opacity: helmetOpacity, transform: helmetTransform }}
          className="pointer-events-none absolute left-1/2 top-[52%] z-0 will-change-transform"
        >
          <div className="relative" style={{ width: "clamp(320px, 40vw, 620px)" }}>
            <div className="absolute -inset-16 rounded-full bg-titans/15 blur-3xl" />
            <img
              src={helmetImg}
              alt=""
              loading="lazy"
              ...
              style={{ filter: "grayscale(0.4) contrast(0.9)" }}
            />
          </div>
        </motion.div>

        {/* Holographic infrastructure */}
        <motion.div style={{ opacity: atmosphereOpacity }} className="absolute inset-0">
          <HoloGrid progress={scrollYProgress} />
          <LightStreaks progress={scrollYProgress} />
        </motion.div>
        <Embers count={16} />

        <div className="relative z-10 mx-auto flex h-full max-w-7xl flex-col justify-center px-6 py-24">
          <motion.div
          ...
          >
            <div>
              <span className="font-mono text-[11px] uppercase tracking-[0.4em] text-titans">
                III · Powered By Those Who Believe
              </span>
              <h2 className="mt-3 font-display text-5xl leading-[0.9] md:text-8xl">
                Our <span className="text-fire">Partners.</span>
              </h2>
            </div>
            <p className="max-w-md text-foreground/70">
              Brands that don't just fund the game — they ride the roar with us, night after night,
              under the same floodlights.
            </p>
          </motion.div>

          {/* 3-layer holographic sponsorship wall */}
          <motion.div
            style={{ y: gridParallax, transformStyle: "preserve-3d", perspective: 1600 }}
            className="grid gap-6 md:grid-cols-3"
          >
            {PARTNERS.map((p, i) => (
              <SponsorPanel key={p.name} partner={p} index={i} progress={scrollYProgress} />
            ))}
          </motion.div>
          ...
          <motion.div
            ...
            transition={{ duration: 0.8, delay: 0.2 }}
            className="mt-12 flex flex-col items-center justify-between gap-4 rounded-3xl border border-titans/30 bg-gradient-to-r from-titans/10 via-jet to-jet p-8 md:flex-row md:p-10"
          >
            <div>
              <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-titans">Partnerships</div>
              <div className="mt-1 font-display text-2xl md:text-3xl">Ride the roar with us.</div>
            </div>
            <a
              href="mailto:partners@tezpurtitans.in"
              className="inline-flex items-center gap-3 rounded-full bg-titans px-7 py-3.5 text-xs font-bold uppercase tracking-[0.25em] text-jet transition hover:shadow-[0_0_60px_oklch(0.72_0.19_55/0.7)]"
            >
              partners@tezpurtitans.in
...
/* ================================================================
   SECTION 04 — CAMPUS AMBASSADOR · "Become a Titan"
================================================================ */
function CampusSection() {
  ...
  return (
    <section id="campus" className="relative min-h-screen overflow-hidden bg-charcoal">
      <div
        className="absolute inset-0"
        style={{
          background:
            "radial-gradient(ellipse at 70% 20%, oklch(0.72 0.19 55 / 0.22), transparent 55%), radial-gradient(ellipse at 20% 80%, oklch(0.55 0.22 32 / 0.18), transparent 55%)",
        }}
      />
      <Embers count={25} />

      <div ref={ref} className="relative mx-auto grid min-h-screen max-w-7xl grid-cols-1 items-center gap-12 px-6 py-24 lg:grid-cols-[1.1fr_1fr]">
        <motion.div initial={{ opacity: 0, x: -60 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.9 }}>
          <span className="inline-flex items-center gap-2 font-mono text-[11px] uppercase tracking-[0.4em] text-titans">
            <GraduationCap className="h-3.5 w-3.5" />
            IV · Become a Titan
          </span>
          <h2 className="mt-3 font-display text-5xl leading-[0.9] md:text-8xl">
            Wear the crest on <span className="text-fire">campus.</span>
          </h2>
          <p className="mt-6 max-w-lg text-base leading-relaxed text-foreground/70">
            Represent Tezpur Titans at your college. Host watch parties. Lead the chant. Get the
            first drop of every kit, backstage passes, mentorship from the squad — and a shot at the
            Titans internship programme.
          </p>

          <ul className="mt-10 grid gap-3 sm:grid-cols-2">
            {[
              "Exclusive Titans kit + merch drops",
              "Match-day backstage access",
              "Mentorship from players & coaches",
              "Certified internship shortlist",
              "Ambassador leaderboard rewards",
              "Direct line to the Titans HQ",
            ].map((perk) => (
              <li key={perk} className="flex items-start gap-3 text-sm text-foreground/80">
                <Sparkles className="mt-0.5 h-4 w-4 shrink-0 text-titans" />
                {perk}
              </li>
            ))}
          </ul>
        ...
        <motion.form
          ...
          className="relative overflow-hidden rounded-3xl border border-white/10 bg-white/[0.04] p-8 backdrop-blur-xl md:p-10"
        >
          <div className="pointer-events-none absolute -inset-px rounded-3xl bg-gradient-to-br from-titans/25 via-transparent to-gold/10" />
          <div className="relative">
            <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-titans">Applications Open · Season 01</div>
            <div className="mt-2 font-display text-3xl leading-tight md:text-4xl">Apply as an Ambassador</div>

            <div className="mt-8 grid gap-4 sm:grid-cols-2">
              <Field label="Full Name" placeholder="Rohan Saikia" />
              <Field label="Email" type="email" placeholder="rohan@campus.edu" />
              <Field label="College" placeholder="Tezpur University" />
              <Field label="Year" placeholder="2nd Year" />
              <div className="sm:col-span-2">
                <Field label="Why should you wear the crest?" placeholder="Tell us in one line…" />
              </div>
            </div>

            <button
              type="submit"
              className="group mt-8 inline-flex w-full items-center justify-center gap-3 rounded-full bg-titans px-8 py-4 text-xs font-bold uppercase tracking-[0.3em] text-jet transition hover:shadow-[0_0_60px_oklch(0.72_0.19_55/0.7)]"
            >
              Submit Application
              <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" />
            </button>
            <p className="mt-4 text-center font-mono text-[10px] uppercase tracking-[0.25em] text-foreground/40">
              Rolling admissions · Response within 7 days
            </p>
          </div>
        </motion.form>
      </div>
...
function Field({ label, ...props }: { label: string } & React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <label className="block">
      <span className="font-mono text-[9px] uppercase tracking-[0.3em] text-foreground/60">{label}</span>
      <input
        {...props}
        className="mt-2 w-full rounded-xl border border-white/10 bg-jet/60 px-4 py-3 text-sm text-foreground placeholder:text-foreground/30 outline-none transition focus:border-titans focus:bg-jet"
      />
    </label>
  );
}
...
function ContactSection() {
  return (
    <section id="contact" className="relative min-h-screen overflow-hidden bg-charcoal">
      <div
        className="absolute inset-0 opacity-70"
        style={{
          background:
            "radial-gradient(ellipse at 20% 30%, oklch(0.72 0.19 55 / 0.18), transparent 55%), radial-gradient(ellipse at 80% 70%, oklch(0.82 0.14 88 / 0.12), transparent 60%)",
        }}
      />
      <Embers count={20} />

      <div className="relative mx-auto grid min-h-screen max-w-7xl grid-cols-1 items-center gap-10 px-6 py-24 lg:grid-cols-[1fr_1.2fr]">
        <div>
          <span className="font-mono text-[11px] uppercase tracking-[0.4em] text-titans">
            VI · Let's Connect
          </span>
          <h2 className="mt-3 font-display text-5xl leading-[0.9] md:text-7xl">
            Talk to the <span className="text-fire">Titans.</span>
          </h2>
          <p className="mt-6 max-w-md text-foreground/70">
            Media, sponsorships, fan support, or just a hello from the stands — every message lands
            in Tezpur.
          </p>

          <div className="mt-10 grid gap-4">
            {[
              { icon: MapPin, label: "Office", value: "Titans HQ, Kolibari Road, Tezpur, Assam 784001" },
              { icon: Mail, label: "Email", value: "hello@tezpurtitans.in" },
              { icon: Phone, label: "Phone", value: "+91 361 000 0000" },
            ].map((c) => (
              <motion.div
                key={c.label}
                ...
                className="group flex items-start gap-4 rounded-2xl border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl transition hover:border-titans/50"
              >
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-titans/15 text-titans transition group-hover:bg-titans group-hover:text-jet">
                  <c.icon className="h-4 w-4" />
                </div>
                <div>
                  <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-titans">{c.label}</div>
            ...
            <div className="mt-2 flex items-center gap-3">
              {[Instagram, Twitter, Youtube, Facebook].map((I, i) => (
                <a key={i} href="#" className="rounded-full border border-white/10 p-3 text-foreground/70 transition hover:border-titans hover:text-titans">
                  <I className="h-4 w-4" />
                </a>
              ))}
            </div>
        ...
        <motion.form
          ...
          className="relative overflow-hidden rounded-3xl border border-white/10 bg-white/[0.04] p-8 backdrop-blur-xl md:p-10"
        >
          <div className="pointer-events-none absolute -inset-px rounded-3xl bg-gradient-to-br from-titans/20 via-transparent to-transparent" />
          <div className="relative">
            <div className="font-display text-3xl md:text-4xl">Send us a message</div>
            <p className="mt-2 text-sm text-foreground/60">We reply within one working day.</p>
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2">
                <label className="block">
                  <span className="font-mono text-[9px] uppercase tracking-[0.3em] text-foreground/60">Message</span>
                  <textarea
                    rows={5}
                    placeholder="Write to the Titans…"
                    className="mt-2 w-full rounded-xl border border-white/10 bg-jet/60 px-4 py-3 text-sm text-foreground placeholder:text-foreground/30 outline-none transition focus:border-titans focus:bg-jet"
                  />
                </label>
              </div>
            ...
            <button
              type="submit"
              className="group mt-6 inline-flex items-center gap-3 rounded-full bg-titans px-8 py-4 text-xs font-bold uppercase tracking-[0.3em] text-jet transition hover:shadow-[0_0_60px_oklch(0.72_0.19_55/0.7)]"
            >
              Send Message
              <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" />
            </button>
    ...
    <footer className="relative overflow-hidden bg-jet">
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-20">
        <div className="grid gap-12 md:grid-cols-[1.4fr_1fr_1fr_1fr]">
          ...
          <FooterCol title="Explore" links={[["Home", "#top"], ["About Us", "#about"], ["Partners", "#partners"]]} />
          <FooterCol title="Community" links={[["Campus Ambassador", "#campus"], ["Join The Titans", "#join"], ["Contact", "#contact"]]} />
          <FooterCol title="Reach" links={[["hello@tezpurtitans.in", "mailto:hello@tezpurtitans.in"], ["partners@tezpurtitans.in", "mailto:partners@tezpurtitans.in"], ["+91 361 000 0000", "tel:+913610000000"]]} />
        </div>
